...
[playse  volume="100"  time="1000"  buf="0"  storage="push39.ogg"  ]
...
;	メニューボタン非表示
	...
	tf.config_x       = [781, 301,　341, 381, 421, 461, 501, 541, 581, 621, 661]; // X座標（共通）

	tf.config_y_bgm   = 170; // BGMのY座標
	tf.config_y_se    = 220; // SEのY座標
	tf.config_y_ch    = 290; // テキスト速度のY座標
	tf.config_y_auto  = 340; // オート速度のY座標
...
;	画面右上の「Back」ボタン
	[button fix="true" graphic="&tf.img_path + 'c_btn_back.png'" enterimg="&tf.img_path + 'c_btn_back2.png'" target="*backtitle" x="840" y="20"]
...
[jump  storage="title.ks"  target="*title_return"  ]
...
[playbgm  volume="100" storage="30_ＢＥＡＣＨ.ogg" ]
...
[playse  volume="100" buf=1 storage="tm2_slidedoor000.ogg" ]
...
;	テキスト表示速度のサンプルに使用するメッセージレイヤの設定
	[position layer="message1" left="42" top="493" width="870" height="100" margint="2" marginl="30" page="fore" visible="true" opacity="0"]
